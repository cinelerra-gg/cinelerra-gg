...
// Calculate default x, y
	if(x < 0 || y < 0)
	{
		...
		// preloads boarder data (if not already loaded)
		//  without reopening display, also avoids a bug in X
		display_info.get_top_border();
	}
...
// Recalculate width based on text
	if(text)
	{
		...
		int new_w = text_w + x + 10, scr_w = get_screen_w(0, -1);
		if(new_w > scr_w) new_w = scr_w;
		if(new_w > get_w()) {
