...
// Used for dvb interface
	VDeviceBase* get_input_base();
...
// colormodel argument should be as close to best_colormodel as possible
	...
	double get_timestamp();      // timestamp last frame
	double device_timestamp();   // device presentation timestamp
...
// record menu auto config update
	void auto_update(double rate, int width, int height);
	int config_updated();
	void config_update();
	int get_iwidth() { return capture_w; }
	int get_iheight() { return capture_h; }
	double get_irate() { return frame_rate; }
